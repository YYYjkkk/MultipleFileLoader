...
def save_to_txt(loader: FilteredCSVLoader, output_txt_path: str):
        """将CSV加载器的结果保存到文本文件中。"""
...
if __name__ == "__main__":
    loader = FilteredCSVLoader(
        file_path="path/to/your/file.csv",  # 替换为您的CSV文件路径
        columns_to_read=["column1", "column2"],  # 指定要读取的列
        source_column="source",  # 可选，源列
        metadata_columns=["metadata1", "metadata2"]  # 可选，元数据列
    )
    output_txt_path = "output.txt"  # 指定输出文本文件路径
    save_to_txt(loader, output_txt_path)
